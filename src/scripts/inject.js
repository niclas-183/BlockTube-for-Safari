(function blockTube() {
  ...
  if (window.__blockTubeInjectLoaded) {
    return;
  }
  window.__blockTubeInjectLoaded = true;
  ...
  function storageReceived(data) {
    ...
    if (storageData.options.enable_javascript && storageData.filterData.javascript) {
      try {
        ...
        jsFilter = compileJavascriptFilter(storageData.filterData.javascript);
  ...
  function compileJavascriptFilter(source) {
    try {
      return window.eval(source);
    } catch (evalError) {
      return window.Function(`return (${source});`)();
    }
  }
